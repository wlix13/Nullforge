...
def _configure_user_shell_profiles(user: str, home_dir: str) -> None:
    ...
    files.template(
        ...
        src="nullforge/templates/profiles/zshrc.j2",
    ...
    if not host.get_fact(File, f"{home_dir}/.config/starship.toml"):
        files.put(
            ...
            src="nullforge/templates/profiles/starship.toml",
    ...
    if not host.get_fact(File, f"{home_dir}/.config/direnv/direnv.toml"):
        files.put(
            ...
            src="nullforge/templates/profiles/direnv.toml",
...
def _install_user_tmux(user: str, home_dir: str) -> None:
    ...
    files.put(
        ...
        src="nullforge/templates/profiles/tmux.conf",
...
def _install_user_nvim(user: str, home_dir: str) -> None:
    ...
    with open("nullforge/templates/nvim/nvim_patch.lua.j2") as f:
        nvim_patch = f.read()
...
def _install_starship(user: str) -> None:
    ...
    distro_name: str = distro["name"]  # type: ignore[reportOptionalMemberAccess]
    distro_major: int = distro["major"]  # type: ignore[reportOptionalMemberAccess]
...
def _install_starship_from_script(user: str) -> None:
    ...
    server.shell(
        ...
        commands=[f"bash {starship_install_path} -s -- -y"],
        _sudo=True,
        _sudo_user=user,
    )
...
def _install_atuin(user: str) -> None:
    ...
    server.shell(
        ...
        commands=[f"bash {atuin_install_path}"],
        _sudo=True,
        _sudo_user=user,
    )
