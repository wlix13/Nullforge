...
class WarpMold(BaseModel):
    ...
    def _validate_ipv6_support(self) -> "WarpMold":
        if self.ipv6_support and self.engine.type is not WarpEngineType.MASQUE:
            raise ValueError("IPv6 support is implemented only for MASQUE engine")
        return self
